...
$countSql = "SELECT (SELECT COUNT(*) FROM deposits WHERE user_id = ?) + 
                    (SELECT COUNT(*) FROM withdrawals WHERE customer_id = ?) AS total";
$countStmt = $connect->prepare($countSql);
$countStmt->bind_param("ss", $_SESSION['customer_id'], $_SESSION['customer_id']);
$countStmt->execute();
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <?php if (count($transactions) < $totalTransactions): ?>
