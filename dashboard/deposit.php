            ...
            <div id="btcAccountInfo" class="mt-2 px-7 py-3 hidden">
                ...
                <div class="bg-gray-100 p-3 rounded-md mb-4">
                    <div class="flex items-center">
                        <p id = "btcAddress"class="text-sm break-all">bc1qfhyn56gswvzp39jalsdemks5urzkdka3exw9f6<p/>
                        <button onclick="copyAddress('btc')" class="ml-2 px-2 py-1 bg-blue-500 text-white rounded">Copy</button>
                    </div>
                </div>
    ...
    <div class="relative top-20 mx-auto p-4 sm:p-5 border w-full max-w-sm sm:max-w-md shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
            ...
            <div id="usdtAccountInfo" class="mt-2 px-7 py-3 hidden">
                ...
                <div class="bg-gray-100 p-3 rounded-md mb-4">
                    <p id = "usdtAddress"class="text-sm break-all">TDC4sPahngQKeQNeXb82hKeMrzCQNDpRmp</p>
                    <button onclick="copyAddress('usdt')" class="ml-2 px-2 py-1 bg-blue-500 text-white rounded">Copy</button>

                </div>
                <p class="text-sm text-gray-500 mb-4">After sending the USDT, click the confirm button below.</p>
            </div>
            <div class="items-center px-4 py-3">
                <button id="confirmUsdtDeposit" class="px-4 py-2 bg-green-600 text-white text-base font-medium rounded-md w-full shadow-sm hover:bg-green-700 transition duration-200 focus:outline-none focus:ring-2 focus:ring-green-300">
                    Confirm Deposit
                </button>
            </div>
            <div class="items-center px-4 py-3">
                <button onclick="closeUsdtDepositModal()" class="px-4 py-2 bg-gray-600 text-white text-base font-medium rounded-md w-full shadow-sm hover:bg-gray-700 transition duration-200 focus:outline-none focus:ring-2 focus:ring-gray-300">
                    Close
                </button>
            </div>
        </div>
...
<script>
function copyAddress(any) {
    // Get the Bitcoin address from the code element
    const btcAddressElement = document.getElementById('btcAddress');
    const btcAddress = btcAddressElement.textContent; // Get the text content of the code element

    const usdtAddressElement = document.getElementById('usdtAddress');
    const usdtAddress = usdtAddressElement.textContent; // Get the text content of the code element
    // Use the Clipboard API to copy the address
    if(any == 'btc'){
        navigator.clipboard.writeText(btcAddress).then(() => {
        // Show a success notification using iziToast
        iziToast.success({
            title: 'Success',
        ...
        iziToast.error({
            title: 'Error',
            message: 'Failed to copy address: ' + err,
            position: 'topRight',
        });
    });
        ...
        iziToast.success({
            title: 'Success',
            message: 'Usdt address copied to clipboard!',
            position: 'topRight',
        });
    }).catch(err => {
        // Show an error notification if copying fails
