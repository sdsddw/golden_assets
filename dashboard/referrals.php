...
$query = "SELECT referral_code FROM customer WHERE Customer_ID = ?";
$stmt = mysqli_prepare($connect, $query);
mysqli_stmt_bind_param($stmt, "s", $currentUserId);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);
$row = mysqli_fetch_assoc($result);
$userReferralCode = $row['referral_code'] ?? '';
$referralLink = $baseUrl . "/sign_up_refferal.php?ref=" . urlencode($userReferralCode);
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div class="bg-gray-100 rounded-lg p-4">
            ...
            <div class="flex items-center mt-2">
                <input type="text" id="referralLink" value="<?php echo htmlspecialchars($referralLink); ?>" readonly class="flex-grow text-sm text-gray-600 bg-white border border-gray-300 rounded-l-md p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
